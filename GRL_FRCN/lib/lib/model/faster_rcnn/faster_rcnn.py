...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.source_weight = cfg.TRANSFER_SOURCE_WEIGHT



    def forward(self, im_data, im_info, gt_boxes, num_boxes, domain = None, l = 0, transfer = False, loss_start = False):
        ...
        # transfer source weight for RPN:
        if cfg.TRANSFER_RPN == True:
            rpn_loss_cls *= self.source_weight
            rpn_loss_bbox *= self.source_weight

        # if it is training phrase, then use ground trubut bboxes for refining
        if self.training:
            roi_data = self.RCNN_proposal_target(rois, gt_boxes, num_boxes, domain, transfer)
        ...
        if self.training:
            ...
            if cfg.TRANSFER_LOSS:
                if loss_start:
                    ...
                    RCNN_loss_cls = F.cross_entropy(cls_score, rois_label)*self.source_weight

                    # bounding box regression L1 loss
                    RCNN_loss_bbox = _smooth_l1_loss(bbox_pred, rois_target, rois_inside_ws, rois_outside_ws)*self.source_weight
